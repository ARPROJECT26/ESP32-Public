...
void compute_day_times(double times[]) {
  ...
  for (int i = 0; i < NUM_ITERATIONS; ++i) {
    // Serial.println("Before compute_times:");
    // for (int i = 0; i < TimesCount; i++) {
    //   Serial.print(TimeName[i]);
    //   Serial.print(": ");
    //   Serial.println(times[i]);
    // } 
    compute_times(times); 
    // Serial.println("After compute_times:");
    // for (int i = 0; i < TimesCount; i++) {
    //   Serial.print(TimeName[i]);
    //   Serial.print(": ");
    //   Serial.println(times[i]);
...
double sun_declination(double jd) {
  DoublePair dp = sun_position(jd);
  Serial.print("Sun Declination: ");
  Serial.println(dp.first);
  Serial.print("Equation of Time: ");
  Serial.println(dp.second);
...
void get_prayer_times(int year, int month, int day, double _latitude, double _longitude, double _timezone, double times[]) {
  ...
  // Serial.println(get_julian_date(year, month, day));
  // Serial.print("Longitude Adjustment: ");
  // Serial.println(longitude / (15.0 * 24.0));
  compute_day_times(times);
  // Serial.println("Computed Times:");
  // for (int i = 0; i < TimesCount; i++) {
  //   Serial.print(TimeName[i]);
  //   Serial.print(": ");
  //   Serial.println(times[i]);
